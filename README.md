...
- [Pipeline Architecture](#pipeline-architecture)
  ...
  - [L5b — FlashRank Cross-Encoder Polish](#l5b--flashrank-cross-encoder-polish)
...
- **Cross-encoder reranking** (FlashRank / `ms-marco-MiniLM-L-12-v2`) for final polish on the top 50
...
│   ├── layers.py                    # Cascade layers L1 → L5b
│   ├── fis.py                       # Mamdani FIS + FlashRank final scoring
...
│   │   ├── flashrank_onnx.tar.gz         # ms-marco-MiniLM-L-12-v2 ONNX
│   │   ├── fraud_kb.tar.gz               # SQLite knowledge base
│   │   └── kenlm_model.tar.gz            # KenLM language model (optional)
...
Surviving candidates are narrowed to the **top 100** before cross-encoder reranking.

---

### L5b — FlashRank Cross-Encoder Polish

**File:** `pipeline/layers.py` → `l5b_flashrank_rerank()`

**Model:** `ms-marco-MiniLM-L-12-v2` (ONNX quantized, via FlashRank)

Reranks the **top 50** candidates using a cross-encoder that jointly encodes the JD query and candidate profile — capturing richer interaction signals than bi-encoder cosine similarity alone.

Final score blend:

```
l5_total_score = 0.25 × flashrank_score + 0.75 × fis_score
```
...
| `flashrank_onnx.tar.gz` | ~1 MB | `ms-marco-MiniLM-L-12-v2` cross-encoder | L5b |
| `fraud_kb.tar.gz` | 68 KB | SQLite company / institution KB | L1 |
| `kenlm_model.tar.gz` | 80 KB | KenLM language model (perplexity, optional) | L1b |
...
| L5b FlashRank rerank (top 50) | ~4s |
