...
def run_preprocessing(input_file, dataset_dir) -> "Path":
    ...
    Clears dataset_dir, runs all 3 passes, builds the Dataset/ tree, and
    returns Path(dataset_dir).  Always re-runs — no caching.

    Args:
        input_file: path to raw candidates.jsonl
        dataset_dir: directory where the 6-level tree will be written
    """
    import shutil
    ...
    # Always wipe and rebuild — no caching
    ...
    with open(input_file, "r", encoding="utf-8") as fh:
        for i, line in enumerate(fh, 1):
            ...
            try:
                ...
                print(f"[preprocess]   ⚠ Skipped line {i}: {e}")
                skipped += 1
                continue
            collect_desc_freq(desc_freq, raw)
            c = process_candidate(raw)
            ...
            if c.get("possible_honeypot"):
                honeypots += 1
            if i % 10_000 == 0:
                print(f"[preprocess]   ... {i:,} processed  ({time.time()-wall_start:.1f}s)")

    print(f"[preprocess] {len(cleaned):,} cleaned | {skipped} skipped | "
          f"{honeypots} honeypots | {time.time()-wall_start:.1f}s")
    ...
    for c in cleaned:
        ...
        if c["possible_fabrication"]:
            fabricated += 1
    print(f"[preprocess] fabrication stamped: {fabricated:,} flagged")

    # Pass 3: build Dataset/ tree
    t = time.time()
    buckets_written = build_tree(cleaned, str(dataset_dir))
    print(f"[preprocess] {buckets_written} leaf files written → {dataset_dir}  "
          f"({time.time()-t:.1f}s)  total={time.time()-wall_start:.1f}s")
