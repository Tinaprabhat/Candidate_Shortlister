    ...
    <div className="page-stack health-page system-status-page">
      ...
      <section className="panel integrity-report system-integrity">
        <div className="section-heading inline">
          ...
          <span>{health?.pipeline_running ? 'Run in progress' : 'From last completed run'}</span>
        </div>
        <blockquote>"{health?.summary || 'Waiting for pipeline diagnostics…'}"</blockquote>
        <div className="diagnostic-grid">
          <article>
            <p>Pipeline Runtime</p>
            <strong>{(health?.elapsed_seconds ?? 0).toFixed(1)}s</strong>
            <ProgressBar value={Math.min(100, ((health?.elapsed_seconds ?? 0) / 300) * 100)} />
          </article>
          <article>
            <p>Model Setup Time</p>
            <strong>{(health?.setup_seconds ?? 0).toFixed(1)}s</strong>
            <ProgressBar
              value={health?.elapsed_seconds ? ((health.setup_seconds ?? 0) / health.elapsed_seconds) * 100 : 0}
              tone="secondary"
            />
          </article>
          <article>
            ...
            <ProgressBar
              value={health?.error_count ? 100 : 0}
              tone={health?.error_count ? 'danger' : 'primary'}
            />
          </article>
        </div>
      </section>
